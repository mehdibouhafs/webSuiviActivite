...
app.controller("countController",function($scope,$http,User){	
	...
	$scope.chargerUser = function(){
		  ...
		  $http({
		      ...
		      url: url1
			   ...
			   var url = "/countActiviterEmployer?email="+$scope.emailUser;
				$http({
				    method: 'GET',
				    url: url
				 }).then(function (success){
 ...
 app.controller("histogrammeController",function($scope,$http){	
 	...
 	app.controller("GenericChartCtrl2",function($scope,$http){		
 	...
 	app.controller("LineChartCtroler",function($scope,$http){	
